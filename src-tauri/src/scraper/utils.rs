...
pub(super) const USER_AGENT_FOR_SCRAPING_INTERNAL_TEST: &str =
    "EindhovenEventViewerInternalTest/0.1 (changeme@example.com)";
// Remove this line:
...
pub(super) fn parse_image_url_from_srcset(
    srcset: &str,
    base_url_for_relative: &str,
) -> Option<String> {
...
pub(super) fn parse_event_datetimes(
    ...
    if let Some(detail_str) = datetime_str_raw_detail_opt {
        if let Some(caps) = detail_re.captures(detail_str) {
            ...
            if let Some(time_caps) = time_only_re.captures(detail_str) {
                if let Some(time_match) = time_caps.get(1) {
                    start_time_str = Some(time_match.as_str().trim().to_string());
                }
                if let Some(time_match) = time_caps.get(2) {
                    end_time_str = Some(time_match.as_str().trim().to_string());
                }
            } else {
                log::warn!(
                    "Detail string '{}' did not match any date/time regex.",
                    detail_str
                );
            }
        }
    ...
    if final_date_str_base.is_none() {
        if let Some(ld_str) = list_date_opt {
            ...
            if let Some(list_caps) = list_date_re.captures(ld_str) {
                ...
                if final_year_override.is_none() {
                ...
                log::warn!(
                    "List date string '{}' did not match expected format.",
                    ld_str
                );
            }
        }
