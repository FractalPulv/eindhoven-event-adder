...
const GridInfoRow: React.FC<{ icon: React.ReactNode; text?: string | null; className?: string; title?: string }> = ({ icon, text, className = "", title }) => {
  if (!text || text === 'N/A') return null;
  return (
    <div className={`flex items-center space-x-1.5 text-xs ${className}`} title={title}>
      <span className="flex-shrink-0 w-3.5 h-3.5 text-gray-500 dark:text-gray-400">{icon}</span>
      <span className="text-gray-600 dark:text-gray-300 leading-snug truncate" style={{ maxWidth: 'calc(100% - 20px)' }}>{text}</span>
    </div>
  );
};

const EventListItem: React.FC<EventListItemProps> = ({ event, onSelectEvent, isSelectedInGrid, isLoadingDetails }) => {
  // For the grid item, always try to show the most precise date/time if available (isDetailed = true)
  const itemDate = event.isDetailed && event.start_datetime 
                  ? formatDate(event.start_datetime) 
                  : (event.list_date || event.date_time_summary || null);
  
  const itemTime = event.isDetailed && event.start_datetime
  ...
  const displayDateTimeOnCard = 
    (itemDate && itemTime) ? `${itemDate} • ${itemTime}` :
    (itemDate || itemTime) ? (itemDate || itemTime) :
    (event.date_time_summary || event.list_date || 'Date/Time N/A');


  const displayLocationOnCard = event.list_specific_location || event.specific_location_name || (event.isDetailed && event.address ? event.address.split(',')[0] : 'Location N/A'); // Show only first part of address for brevity
  const displayPriceOnCard = event.list_price || (event.isDetailed && event.price) || 'Price N/A';
  ...
  return (
    <div
      className={`group relative flex flex-col overflow-hidden rounded-xl bg-white dark:bg-slate-800/80 backdrop-blur-sm
                  ...
                  ${isSelectedInGrid ? 'ring-2 ring-blue-500 dark:ring-blue-400' : 'border border-gray-200 dark:border-slate-700/70'}
                  ${isLoadingDetails && isSelectedInGrid ? 'opacity-60 animate-pulse' : ''}`} // Animate if this item is loading for overlay
      onClick={!(isLoadingDetails && isSelectedInGrid) ? () => onSelectEvent(event) : undefined}
      tabIndex={0}
    ...
    >
      ...
      <div className="p-3 sm:p-4 flex-grow flex flex-col justify-between">
        <div>
            ...
            <div className="space-y-1 text-xs"> {/* Reduced space */}
                {displayDateTimeOnCard !== 'Date/Time N/A' && <GridInfoRow icon={<CalendarIcon />} text={displayDateTimeOnCard} title={event.datetime_str_raw_detail || "Date and Time"} />}
                {displayLocationOnCard !== 'Location N/A' && <GridInfoRow icon={<PinIcon />} text={displayLocationOnCard} title="Location" />}
                {displayPriceOnCard !== 'Price N/A' && <GridInfoRow icon={<EuroIcon />} text={displayPriceOnCard} className="text-green-600 dark:text-green-400" title="Price"/>}
            </div>
        </div>

        {/* Short description can still be shown if available and not selected for overlay */}
        {event.short_description && (!isSelectedInGrid || !event.isDetailed) && ( 
